...
class TLEnv(baseEnv):
    ...
    def get_state(self):
        phase = list()
        state = torch.zeros((2,8),device=self.configs['device'],dtype=torch.int) #기준
        vehicle_state = torch.zeros((8,1),device=self.configs['device'],dtype=torch.int)
        ...
        for i,interest in enumerate(self.interest_list):
            vehicle_state[2*i+1]= traci.edge.getLastStepVehicleNumber(interest['inflow'])# 죄회전용 추가 필요
        vehicle_state=torch.transpose(vehicle_state,0,1)
        state=torch.cat((vehicle_state,phase_state),dim=1)
        print(state.size())
        return state
    ...
    def collect_state(self):
        ...
        for _,interest in enumerate(self.interest_list):
            inflow_rate += traci.edge.getLastStepVehicleNumber(interest['inflow'])
            outflow_rate += traci.edge.getLastStepVehicleNumber(interest['outflow'])
    ...
    def _toState(self, phase):  # env의 phase를 해석불가능한 state로 변환
        state = torch.zeros((8,1),device=self.configs['device'], dtype=torch.int16)
        for i in range(4):  # 4차로
            phase = phase[:][1:]  # 우회전
