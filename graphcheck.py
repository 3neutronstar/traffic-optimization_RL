...
def main(args):
    ...
    writer=list()
    interval=list()
    writer.append(SummaryWriter(os.path.join('tensorboard',flags.type,time_str,flags.file_a)))
    writer.append(SummaryWriter(os.path.join('tensorboard',flags.type,time_str,flags.file_b)))
        ...
        if flags.type=='edge':
            for i,t in enumerate(end_time):
                ...
                for j,id in enumerate(id_list[idx]):
                    writer[idx].add_scalar('{}/{}'.format(showData,id),speed_list[idx][j],t)
                writer[idx].flush()
            ...
            for i,t in enumerate(end_time):
                ...
                for _,edge in enumerate(edge_list[idx]):
                    ...
                    for _,lane in enumerate(lane_list):
                        if showData in lane.keys():
                            speed_dict[lane.attrib['id'][-1]]=float(lane.attrib[showData])
                        else:
                ...
                for j,id in enumerate(id_list[idx]):
                    for k in range(len(speed_dict.keys())):
                        writer[idx].add_scalar('{}/{}/{}'.format(showData,id,k),speed_list[idx][j][str(k)],t)
                writer[idx].flush()
